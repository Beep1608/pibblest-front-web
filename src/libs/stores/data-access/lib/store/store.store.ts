...
import { CreateStoreDto, StorePaginationResponse, StorePreview } from "../models/store.model";
              ...
              tapResponse({
                ...
                error: (error: any) => {
                  patchState(store, {
                    ...
                    error:
                      error.message ||
                      'Ocurrion un error al cargar las tiendas',
                  });
                },
              }),
            );
            ...
            return api.listenToStoreStream(currentToken).pipe(
              tap((storeUpdate) => {
                ...
                patchState(store, (state) => {
                  ...
                  const updatedStores = state.storesPage.stores.map(
                    (tiendaActual) =>
                      tiendaActual.id === storeUpdate.id
                        ? { ...tiendaActual, ...storeUpdate }
                        : tiendaActual,
                  );
